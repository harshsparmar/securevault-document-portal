    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex items-center">
                <!-- Logo -->
                <a href="{{ route('documents.index') }}" class="flex items-center gap-2.5">
                    <x-application-logo class="block h-9 w-9" />
                    <span class="nav-logo-text text-lg hidden sm:inline">SecureVault</span>
                </a>

                <!-- Navigation Links -->
                <div class="hidden sm:flex sm:items-center sm:ml-8 space-x-1">
                    <x-nav-link :href="route('documents.index')" :active="request()->routeIs('documents.index')" class="px-3 py-2 rounded-lg text-sm font-medium">
                    ...
                    @if(auth()->user()->isUploader())
                        <x-nav-link :href="route('documents.create')" :active="request()->routeIs('documents.create')" class="px-3 py-2 rounded-lg text-sm font-medium">
            ...
            <div class="hidden sm:flex sm:items-center gap-2">
                <!-- Dark Mode Toggle -->
                <button @click="$store.darkMode.toggle()" class="dark-toggle" title="Toggle dark mode">
            ...
            <div class="-me-2 flex items-center sm:hidden gap-1">
                ...
                <button @click="$store.darkMode.toggle()" class="dark-toggle">
                    <svg x-show="$store.darkMode.on" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"/>
                    </svg>
                    <svg x-show="!$store.darkMode.on" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
